...
class EqualTo(object):
    ...
    def __call__(self, form, field):
        ...
        if field.data != other.data:
            d = {
                'other_label': (
                    hasattr(other, 'label') and other.label.text or
                    self.fieldname
                ),
            ...
            if message is None:
                message = field.gettext(
                    '%(field_label)s must be equal to %(other_name)s.'
                )
...
class Length(object):
    ...
    def __call__(self, form, field):
        ...
        if l < self.min or self.max != -1 and l > self.max:
            ...
            if message is None:
                if self.max == -1:
                    self.message = field.ngettext(
                        '%(field_label)s must be at least %(min)d character'
                        ' long.',
                        '%(field_label)s must be at least %(min)d characters'
                        ' long.',
                        self.min
                    )
                    ...
                    self.message = field.ngettext(
                        '%(field_label)s cannot be longer than %(max)d'
                        ' character.',
                        '%(field_label)s cannot be longer than %(max)d'
                        ' characters.',
                        self.max
                    ...
                    self.message = field.gettext(
                        '%(field_label)s must be between %(min)d and %(max)d '
                        'characters long.'
                    )
    ...
    def __call__(self, form, field):
        ...
        if (data is None or (self.min is not None and data < self.min) or
            ...
            if self.message is None:
                ...
                if self.max is None:
                    self.message = field.gettext(
                        '%(field_label)s must be at least %(min)s.'
                    )
                elif self.min is None:
                    self.message = field.gettext(
                        '%(field_label)s must be at most %(max)s.'
                    )
                else:
                    self.message = field.gettext(
                        '%(field_label)s must be between %(min)s and %(max)s.'
                    )
...
class DataRequired(object):
    ...
    def __call__(self, form, field):
        if (not field.data or isinstance(field.data, string_types) and
                not field.data.strip()):
            if self.message is None:
                self.message = field.gettext('%(field_label)s is required.')

            field.errors[:] = []
            raise StopValidation(
                self.message % dict(field_label=field.label.text)
            )
...
class Required(DataRequired):
    ...
    This class will start throwing deprecation warnings in WTForms 1.1 and be
    removed by 1.2.
    """
...
class IPAddress(object):
    ...
    def __call__(self, form, field):
        ...
        if not valid:
            ...
            raise ValidationError(
                message % dict(field_label=field.label.text)
            )
