...
class DummyField(object):
    ...
    def __init__(self, data, errors=(), raw_data=None, label=''):
        self.data = data
        self.errors = list(errors)
        self.raw_data = raw_data
        self.label = Label(1, label)  # dummy label
...
class ValidatorsTest(TestCase):
    ...
    def test_email_error_field_label_placeholder(self):
        try:
            email('%(field_label)s is not valid email.')(
                self.form, DummyField(' ', label='Some field')
            )
        except ValidationError, e:
            self.assertEqual(e.message, 'Some field is not valid email.')

    def test_ip_address_error_field_label_placeholder(self):
        try:
            ip_address(message='%(field_label)s is not valid ip address.')(
                self.form, DummyField(' ', label='Some field')
            )
        except ValidationError, e:
            self.assertEqual(e.message, 'Some field is not valid ip address.')

    def test_equal_to_error_field_label_placeholder(self):
        self.form['foo'] = DummyField('test')
        try:
            equal_to(
                'foo',
                message='%(field_label)s is not equal to %(other_name)s')(
                    self.form, DummyField(' ', label='Some field')
                )
        except ValidationError, e:
            self.assertEqual(
                e.message,
                'Some field is not equal to foo'
            )

    def test_length_error_field_label_placeholder(self):
        try:
            length(message='%(field_label)s is too long.', min=3, max=10)(
                self.form, DummyField(' ', label='Some field')
            )
        except ValidationError, e:
            self.assertEqual(e.message, 'Some field is too long.')

    def test_number_range_field_label_placeholder(self):
        try:
            NumberRange(
                message='%(field_label)s is not within number range.',
                min=3,
                max=10)(
                    self.form, DummyField(' ', label='Some field')
                )
            ...
            self.assertEqual(
                e.message, 'Some field is not within number range.'
            )

    def test_required_field_label_placeholder(self):
        try:
            required(
                message='%(field_label)s is required.')(
                    self.form, DummyField(' ', label='Some field')
                )
        except StopValidation, e:
            self.assertEqual(
                e.message, 'Some field is required.'
            )

    def test_anyof_field_label_placeholder(self):
        try:
            AnyOf(
                [1, 2, 3],
                message='%(field_label)s is not valid.')(
                    self.form, DummyField(4, label='Some field')
                )
        except ValidationError, e:
            self.assertEqual(
                e.message, 'Some field is not valid.'
            )

    def test_noneof_field_label_placeholder(self):
        try:
            NoneOf(
                [1, 2, 3],
                message='%(field_label)s is not valid.')(
                    self.form, DummyField(2, label='Some field')
                )
        except ValidationError, e:
            self.assertEqual(
                e.message, 'Some field is not valid.'
